...
export default function ProjectViewer({ projectId, onClose }: ProjectViewerProps) {
  ...
  if (loading) {
    return (
      <div className="bg-[#171717] rounded-md border border-[#444444] p-6">
        <div className="flex flex-col space-y-4">
          <div className="h-8 bg-[#1f1f1f] rounded-md w-1/3 animate-pulse"></div>
          <div className="h-4 bg-[#1f1f1f] rounded-md w-full animate-pulse"></div>
          <div className="h-4 bg-[#1f1f1f] rounded-md w-full animate-pulse"></div>
          <div className="h-32 bg-[#1f1f1f] rounded-md w-full animate-pulse"></div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-[#171717] rounded-md border border-[#444444] p-6">
        <div className="bg-red-500/10 border border-red-500/20 text-red-500 p-4 rounded-md flex items-center justify-between">
          ...
          <button onClick={onClose} className="px-3 py-1.5 bg-[#1f1f1f] hover:bg-[#2a2a2a] text-white rounded-md text-sm transition-colors">Go Back</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#171717] rounded-md border border-[#444444]">
      <div className="p-6">
        <div className="flex justify-between items-start mb-6">
          ...
          <button onClick={onClose} className="p-2 text-gray-400 hover:text-white rounded-md hover:bg-[#1f1f1f] transition-colors">
        ...
        <div className="border-b border-[#444444] mb-6">
          <div className="flex space-x-6">
            <button 
              className={`text-sm pb-3 px-1 font-medium transition-colors ${activeTab === "details" ? "text-white border-b-2 border-[#39a276]" : "text-gray-400 hover:text-white"}`}
              onClick={() => setActiveTab("details")}
            >
              Details
            </button>
            <button 
              className={`text-sm pb-3 px-1 font-medium transition-colors ${activeTab === "tests" ? "text-white border-b-2 border-[#39a276]" : "text-gray-400 hover:text-white"}`}
              onClick={() => setActiveTab("tests")}
            >
              A/B Tests
            </button>
            <button 
              className={`text-sm pb-3 px-1 font-medium transition-colors ${activeTab === "settings" ? "text-white border-b-2 border-[#39a276]" : "text-gray-400 hover:text-white"}`}
                ...
                {project.site_url && (
                  <div className="bg-[#1f1f1f] rounded-md p-4 border border-[#444444]">
                    ...
                    <a 
                      ...
                      className="text-[#39a276] hover:underline truncate block"
                    >
                      {project.site_url}
                    </a>
                  </div>
                ...
                {project.repo_url && (
                  <div className="bg-[#1f1f1f] rounded-md p-4 border border-[#444444]">
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="text-[#39a276] hover:underline truncate flex items-center"
                    >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 w-full max-w-md">
                ...
                <div>
                  ...
                  <textarea 
                    className="w-full bg-[#1f1f1f] border border-[#444444] rounded-md px-3 py-2 text-white h-24 focus:outline-none focus:ring-1 focus:ring-[#39a276] focus:border-[#39a276]" 
              ...
              <div className="flex gap-3 pt-4">
                <button className="px-4 py-2 bg-[#39a276] text-white text-sm font-medium rounded-md hover:bg-opacity-90 transition-colors">
                  Save Changes
                </button>
